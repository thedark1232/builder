...
local build_dir_table = {"chiken", "castle_shop", "chrismas_tree_5and5", "home01", "tower", "angel", "ship1", "ship2", "pvp_spawn", "vitruvian_castle", "ImperialCaphedral", "tank01", "castleShop2", "castle_shop_wool"}
...
local ignore_files = {["energyChecker.lua"] = "ok", ["moveLibrary.lua"] = "ok", ["sizeLibrary.lua"] = "ok", ["table_planks.lua"] = "ok", ["filesLibrary.lua"] = "ok", ["algorithmLi.lua"] = "ok"}
...
local moveLibrary, table_sides, energyChecker, filesLibrary, algorithmLi --библиотеки
...
function build_stairs(metadata, stairs_slot_num, g_key_slot_num)
	...
	if metadata >= 4 then
		...
		robot.select(1)
		::try_again2::
		while geo.analyze(0).name == "OpenComputers:robotAfterimage" do robot.useDown() end
		robot.useDown()
		computer.beep(400, 0.1)
		...
		if geoAnalyze == "minecraft:air" or geoAnalyze == "Thaumcraft:blockAiry" or geoAnalyze == "minecraft:water" or geoAnalyze == "minecraft:flowing_water" then
			computer.beep(400, 2); deb("ЖДУ 3 СЕКУНДЫ", 3); zagepJka = zagepJka + 1
		...
		if geoAnalyze2 == "minecraft:air" or geoAnalyze2 == "Thaumcraft:blockAiry" or geoAnalyze2 == "minecraft:water" or geoAnalyze2 == "minecraft:flowing_water" then computer.beep(400, 2); moveLibrary.z_move(z + 1); deb("ЖДУ 3 СЕКУНДЫ", 3); zagepJka = zagepJka + 1 goto try_again2 end
		computer.beep(1000, 0.1)
		inv.equip()
		robot.select(g_key_slot_num)
		inv.equip()
...
function return_and_find_item(block_name, meta, check_meta)
	...
	if second_check == first_check then
		...
		while robot.count(slot_number) < 59 do term.clear(); print("количество блоков собрано: " ..robot.count(slot_number).. " из 59"); computer.beep(1000, 0.1); end
	end
	robot.suckUp(1)
	term.clear()
	...
	while geo.analyze(0).name ~= "minecraft:air" do
		...
		if analyze.name == block_name and analyze.metadata == meta_num then
			...
			if second_check2 == first_check2 then
				...
				while robot.count(1) < 59 do computer.beep(1000, 0.1); term.clear(); print("всего блоков: " ..robot.count(1).. " из 59") end
			end
			robot.suckUp(1)
